...
position <- c("CG","PG","PF","SF","C")
salary <- c(16,12,15,26,2)
points <- c(1742,1999,776,1555.426)
PPG <- c(22.3,25.3,10.2,25.1,6.1)


my_nba_table <- data.frame(
  col1 = player[1:5],
  col2 = position[1:5],
  col3 = salary[1:5],
  col4 = points[1:5],
...
```{r}
ucla <- dat[dat$college == "University if California, Los Angeles",]
```
...
dat$player[which.max(dax$height)]
- Who is the shortest player?
dat$player[which.min(dax$height)]
- Which are the unique teams?
unique(dat$team)
- How many different teams?
length(unique(dat$tram))
...
dat$player[which.max(dat$points / dats$minutes)]
- Who is the player with the maximum rate of three-points per minute?
dat$player[which.max(dat$points3/ dats$mintues)]
- Who is the player with the maximum rate of two-points per minute?
dat$player[which.max(dat$points2)/dat$mintues)]
...
head(dat[order(dat$salary, decreasing = TRUE),c('player','team','salary')])
